...
double scp_height = 0.0;
double zero_scp_height=0.0;
double zero_bmp_height=0.0;
double scp_pressure = 0;
double lidar_lite_distance = 0.0;

long pni_value[3] = {0,0,0};
unsigned char scp_status = 0;
unsigned char scp_opstatus = 0;
...
*/
unsigned int scp1000_init(void)
{
  unsigned int comres=0;
  unsigned char cle_reg_data = High_speed;
  //unsigned char cle_reg_data1 = 0x00;
  //unsigned char cle_reg_data1 = 0x14;
  //comres = p_bmp085->BMP085_BUS_WRITE_FUNC(AT91C_BASE_TWI,SPC1000AddrW | AT91C_TWI_IADRSZ_1_BYTE, OPERATION, &cle_reg_data1, 1);
    comres += p_bmp085->BMP085_BUS_READ_FUNC(AT91C_BASE_TWI,SPC1000AddrR | AT91C_TWI_IADRSZ_1_BYTE, STATUS,&scp_status, 1);
  while(scp_status & 0x01)//确定进入standby模式
  {
    comres += p_bmp085->BMP085_BUS_READ_FUNC(AT91C_BASE_TWI,SPC1000AddrR | AT91C_TWI_IADRSZ_1_BYTE, STATUS,&scp_status, 1);
  }
   if(scp_status & 0x20 != 0)//写入模式时确认DRDY没置位，有则读取气压数值去除
  {
      scp1000_pressure(&scp_pressure);
  }
  comres = p_bmp085->BMP085_BUS_WRITE_FUNC(AT91C_BASE_TWI,SPC1000AddrW | AT91C_TWI_IADRSZ_1_BYTE, OPERATION, &cle_reg_data, 1);//写入模式
  ...
  while(scp_status & 0x20 == 0)
  {
     comres += p_bmp085->BMP085_BUS_READ_FUNC(AT91C_BASE_TWI,SPC1000AddrR | AT91C_TWI_IADRSZ_1_BYTE, STATUS,&scp_status, 1);   
  }
     scp1000_pressure(&scp_pressure);
   /*comres += p_bmp085->BMP085_BUS_READ_FUNC(AT91C_BASE_TWI,SPC1000AddrR | AT91C_TWI_IADRSZ_1_BYTE, OPSTATUS,&scp_opstatus, 1);
  while(scp_opstatus & 0x01)
  {
    comres += p_bmp085->BMP085_BUS_READ_FUNC(AT91C_BASE_TWI,SPC1000AddrR | AT91C_TWI_IADRSZ_1_BYTE, OPSTATUS,&scp_opstatus, 1);
    flag = 1;
  }*/
...
unsigned int scp1000_pressure(double *p)
{
    
  unsigned  int comres=0;
  unsigned char mPress[3];
  unsigned int m1=0;
  unsigned long m2=0;
    //float m3=0;

   comres += p_bmp085->BMP085_BUS_READ_FUNC(AT91C_BASE_TWI,SPC1000AddrR | AT91C_TWI_IADRSZ_1_BYTE, DATARD8,mPress, 1); //获取气压数据高字节：3位
   comres += p_bmp085->BMP085_BUS_READ_FUNC(AT91C_BASE_TWI,SPC1000AddrR | AT91C_TWI_IADRSZ_1_BYTE, DATARD16,mPress+1, 2);//获取气压数据中 & 低字节 

    *p = 0;
    
    m2 = (unsigned long)(mPress[0] & 0x07);
    m2 = m2<<16;
    m1=(mPress[1] << 8) + mPress[2];
    m2|=(unsigned long)m1;
    *p=m2/4;
    //*pPress = (((mPress[0] & 0x07)<< 16)+(mPress[1] << 8) + mPress[2])/4;
 //mTemperature[0]=0x27;
 //mTemperature[1]=0x27;
    return comres;
}

void lidar_lite_update(double *distance)
{ 
  unsigned char temp[2];
  p_bmp085->BMP085_BUS_READ_FUNC(AT91C_BASE_TWI,(0x40<<16)|AT91C_TWI_IADRSZ_NO,0,temp, 2);//获取气压数据中 & 低字节 
  *distance = -(temp[0]*256 + temp[1])*0.001;
}
...
double height_average_filter_for_scp1000(double temp2)//气压计高度平均值滤波
{
    static double bmp_h_window[15];
    double bmp_sum_h = 0;
    static int bmp_length = 1;
    double bmp_height_filter;
    for(char i=1; i<15; i++) //for sum_e and e_window
    {
    	bmp_sum_h += bmp_h_window[i];
        bmp_h_window[i-1] = bmp_h_window[i];
    }
	 //   e_window[8-1]=e_speed;sum_e += e_speed;
       bmp_h_window[15-1]=temp2;
       bmp_sum_h += temp2;
       bmp_height_filter = bmp_sum_h/bmp_length;
       
      if(bmp_length >= 15)//for length ,max = N
         bmp_length = 15;
      else bmp_length++;
...
void scp1000_update(void)
{ 
  static unsigned int zero_count = 1 ;
  scp_height = 44330*(1-pow((scp_pressure/101325.0),0.190294957));
  if(zero_count==1) zero_scp_height=scp_height;     
    zero_count = 2 ;
  scp_height = zero_scp_height - scp_height;
  bmp_height = scp_height;
}

void hmc_update(void)
{
  char * line=NULL ; 
  line = index(hmc_save_line,',');
  if(!line)
      return ;
  line++;//skip ,
  pni_value[ INDEX_CX ]  = strtod( line, &line );
  
    line++;//skip ,
  pni_value[ INDEX_CY ]  = strtod( line, &line );
  
    line++;//skip ,
  pni_value[ INDEX_CZ ]  = strtod( line, &line );
...
/** timer0中断函数
    ...
    该中断函数是2014年旧版飞控板的中断函数，旧版上面有bmp085和hmc传感器，没有scp1000
*/
...
//                         break;
//              case 5:   if(hmc_update_flag==true)
//                        {
//                            hmc_update();
//                            hmc_update_flag = false;
//                        }
//                        bmphmc_num++;
...
/** timer0中断函数
    ...
    该中断函数是2015年新版飞控板的中断函数，更新imu与scp1000
*/
void timer0_c_irq_handler(void)
{
  int dummy;
          ...
          switch(scp_num)
            {
               ...
               case 0 : 
                        ...
                        comres += p_bmp085->BMP085_BUS_READ_FUNC(AT91C_BASE_TWI,SPC1000AddrR | AT91C_TWI_IADRSZ_1_BYTE, STATUS,&scp_status, 1);
                        scp1000_pressure(&scp_pressure);
                        lidar_lite_update(&lidar_lite_distance);
                        scp_num++;
                        AT91C_BASE_TC0->TC_RA = AT91C_BASE_TC0 -> TC_CV+350;
                        break; 
              default : break;
